...
if __name__ == "__main__":
    ...
    print('extracting ' + dataset + '\'s image feature from ' + model_name + ' from ' + args.checkpoint_path) 
    model = timm.create_model(
        model_name, 
        pretrained=True, 
        num_classes=0, 
    )
    timm.models.load_checkpoint(model, args.checkpoint_path, strict=False)
    model = model.to('cuda:0') # if use cpu, uncomment '.to('cuda:0')'
    model.eval()
